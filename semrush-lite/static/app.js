...
// ==========================================
const DASH_CACHE_KEY = "pexora_ai_dashboard_cache";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Load user profile (avatar, name, email) into sidebar
    loadUserProfile();
...
function switchTab(tabId) {
    ...
    // Toggle GSC status badge next to Search Console title in top-bar
    const gscStatusBadge = document.getElementById("auth-status-container");
    if (gscStatusBadge) {
        gscStatusBadge.style.display = (tabId === "gsc-tab") ? "inline-flex" : "none";
    }
...
// Load user profile info into sidebar
...
// Sign out user completely (redirect to landing page)
function logoutUser() {
...
// Check Google Search Console Auth Status (separate from login)
async function checkAuthStatus() {
    try {
        ...
        const navDot = document.getElementById("nav-gsc-dot");
        
        if (data.authenticated) {
            // GSC connected
            if (badgeContainer) badgeContainer.innerHTML = `<span class="auth-badge online"><i class="dot"></i> GSC Connected</span>`;
            if (navDot) {
                navDot.className = "nav-gsc-dot online";
                navDot.title = "GSC Connected";
            }
            ...
            // GSC not connected
            if (badgeContainer) badgeContainer.innerHTML = `<span class="auth-badge offline"><i class="dot"></i> GSC Not Connected</span>`;
            if (navDot) {
                navDot.className = "nav-gsc-dot offline";
                navDot.title = "GSC Not Connected";
            }
...
// Disconnect Google Search Console (does not log out user)
async function logoutGoogle() {
    if (confirm("Are you sure you want to disconnect from Google Search Console?")) {
        await fetch("/api/auth/disconnect-gsc");
        checkAuthStatus();
    }
}
...
const PLATFORM_COLORS = {
    ...
    'WhatsApp': { bg: 'rgba(37,211,102,0.12)', border: 'rgba(37,211,102,0.3)', color: '#25D366', icon: '💬' }
};
...
function renderSocialResults(platformsData) {
    ...
    for (const [platform, content] of Object.entries(platformsData)) {
        ...
        <div class="social-result-card card mt-20" style="border-left: 3px solid ${colors.color};">
            ...
            ${platform === 'WhatsApp' ? `
            <!-- WhatsApp Business Connection + Send -->
            <div class="social-section" style="border-top: 1px solid var(--border-glass); padding-top: 16px;">
                <div id="wa-connection-card" style="margin-bottom: 14px;">
                    <div class="wa-connection-status disconnected">
                        <div class="wa-status-indicator">
                            <div class="wa-status-dot disconnected"></div>
                            <span class="wa-status-label">Checking...</span>
                        </div>
                    </div>
                </div>
                <button class="btn btn-lg whatsapp-send-btn" id="whatsapp-send-btn" onclick="sendViaWhatsApp()">
                    <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" style="flex-shrink:0;"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
    ...
    lucide.createIcons();
    // Update WhatsApp connection card if WhatsApp results exist
    if (document.getElementById('wa-connection-card')) {
        renderWhatsAppConnectionUI(waConnectionState.connected);
    }
}
...
// ====================================================
// WhatsApp Business API Integration (Embedded Signup + Cloud API)
// ====================================================

let waConnectionState = { connected: false, waba_id: '', phone_number_id: '', display_phone: '', business_name: '' };

// --- Embedded Signup: postMessage listener for WABA info ---
    ...
    try {
        ...
        if (data.type === 'WA_EMBEDDED_SIGNUP') {
            if (data.event === 'FINISH') {
                ...
                console.log('[WhatsApp Signup] Finished. WABA:', waba_id, 'Phone:', phone_number_id);
                // Store temporarily for use when FB.login callback fires
                window._waSignupData = { waba_id, phone_number_id };
            } else if (data.event === 'CANCEL') {
                console.log('[WhatsApp Signup] User cancelled.');
            }
        }
    } catch (e) {
...
function launchWhatsAppSignup() {
    ...
    FB.login((response) => {
        if (response.authResponse) {
            ...
            console.log('[WhatsApp Signup] Auth code received');
            
            // Get WABA info from postMessage (stored in window._waSignupData)
            const signupData = window._waSignupData || {};
            const waba_id = signupData.waba_id || '';
            const phone_number_id = signupData.phone_number_id || '';
            
            if (!waba_id || !phone_number_id) {
                alert('WhatsApp signup completed but WABA information was not received. Please try again.');
                return;
            }
            
            handleWhatsAppConnect(code, waba_id, phone_number_id);
        } else {
            console.log('[WhatsApp Signup] User cancelled or did not authorize.');
        }
    }, {
        config_id: '1064463519644649',
        response_type: 'code',
...
// --- Exchange code + store credentials on backend ---
async function handleWhatsAppConnect(code, wabaId, phoneNumberId) {
    // Show loading state in the connection UI
    const statusEl = document.getElementById('wa-connection-card');
    if (statusEl) {
        statusEl.innerHTML = `
            <div style="display: flex; align-items: center; gap: 10px; padding: 12px;">
                <div class="spinner-small"></div>
                <span style="color: var(--color-text-muted);">Connecting your WhatsApp Business Account...</span>
            </div>`;
    }
    ...
    try {
        ...
        if (data.error) {
            alert('WhatsApp connection failed: ' + data.error);
            renderWhatsAppConnectionUI(false);
            return;
        }
        ...
        renderWhatsAppConnectionUI(true);
...
async function checkWhatsAppStatus() {
    try {
        ...
        renderWhatsAppConnectionUI(data.connected);
    } catch (err) {
        console.error('[WhatsApp Status] Error:', err);
    }
}

// --- Render connection status card in the WhatsApp result section ---
function renderWhatsAppConnectionUI(isConnected) {
    const cardEl = document.getElementById('wa-connection-card');
    if (!cardEl) return;
    
    if (isConnected) {
        const phone = waConnectionState.display_phone || 'Unknown';
        const biz = waConnectionState.business_name || 'WhatsApp Business';
        cardEl.innerHTML = `
            <div class="wa-connection-status connected">
                <div class="wa-status-indicator">
                    <div class="wa-status-dot connected"></div>
                    <span class="wa-status-label">Connected</span>
                </div>
                <div class="wa-account-info">
                    <strong>${escapeHtml(biz)}</strong>
                    <span>${escapeHtml(phone)}</span>
                </div>
                <button class="btn btn-secondary btn-sm" onclick="disconnectWhatsApp()" title="Disconnect">
        ...
        cardEl.innerHTML = `
            <div class="wa-connection-status disconnected">
                <div class="wa-status-indicator">
                    <div class="wa-status-dot disconnected"></div>
                    <span class="wa-status-label">Not Connected</span>
                </div>
                <button class="btn btn-sm" onclick="launchWhatsAppSignup()" style="background: linear-gradient(135deg, #25D366, #128C7E); color: #fff; border: none; font-weight: 600;">
                    <i data-lucide="link"></i> Connect WhatsApp Business
                </button>
            </div>`;
    }
...
async function disconnectWhatsApp() {
    ...
    try {
        ...
        const data = await resp.json();
        if (data.success) {
            waConnectionState = { connected: false, waba_id: '', phone_number_id: '', display_phone: '', business_name: '' };
            renderWhatsAppConnectionUI(false);
        }
    } catch (err) {
...
function sendViaWhatsApp() {
    const modal = document.getElementById('whatsapp-send-modal');
    if (!modal) return;
    
    // Populate connection status in modal
    const modalStatusEl = document.getElementById('wa-modal-connection-status');
    const sendFormEl = document.getElementById('wa-send-form');
    const sendResultEl = document.getElementById('wa-send-result');
    ...
    if (waConnectionState.connected) {
        if (modalStatusEl) {
            modalStatusEl.innerHTML = `
                <div class="wa-connection-status connected" style="padding: 10px 14px;">
                    <div class="wa-status-indicator">
                        <div class="wa-status-dot connected"></div>
                        <span class="wa-status-label">Connected</span>
                    </div>
                    <div class="wa-account-info">
                        <strong>${escapeHtml(waConnectionState.business_name || 'WhatsApp Business')}</strong>
                        <span>${escapeHtml(waConnectionState.display_phone || '')}</span>
                    </div>
                </div>`;
        }
        if (sendFormEl) sendFormEl.style.display = 'block';
    } else {
        if (modalStatusEl) {
            modalStatusEl.innerHTML = `
                <div class="wa-connection-status disconnected" style="padding: 16px; text-align: center;">
                    <p style="color: var(--color-text-muted); margin-bottom: 12px;">Connect your WhatsApp Business Account to send messages via Cloud API.</p>
                    <button class="btn" onclick="launchWhatsAppSignup()" style="background: linear-gradient(135deg, #25D366, #128C7E); color: #fff; border: none; font-weight: 600; padding: 10px 24px;">
                        <i data-lucide="link"></i> Connect WhatsApp Business
                    </button>
                </div>`;
        }
        if (sendFormEl) sendFormEl.style.display = 'none';
    }
    
    // Populate message preview from generated content
    const postEl = document.getElementById('WhatsApp-post');
    const hashtagsEl = document.getElementById('WhatsApp-hashtags');
    const imageContainer = document.getElementById('WhatsApp-image');
    
    let messageText = '';
    if (postEl) messageText += postEl.innerText;
    if (hashtagsEl) {
        const pills = hashtagsEl.querySelectorAll('.hashtag-pill');
        const tags = Array.from(pills).map(p => p.textContent).join(' ');
        if (tags) messageText += '\n\n' + tags;
    }
    ...
    // Check for image
    const imgEl = imageContainer ? imageContainer.querySelector('img.social-generated-image') : null;
    const imgPreviewSection = document.getElementById('wa-image-preview-section');
    const imgPreviewThumb = document.getElementById('wa-image-preview-thumb');
    
    if (imgEl && imgEl.src) {
        if (imgPreviewSection) imgPreviewSection.style.display = 'block';
        if (imgPreviewThumb) imgPreviewThumb.src = imgEl.src;
    } else {
        if (imgPreviewSection) imgPreviewSection.style.display = 'none';
    }
...
// --- Execute the actual send via Cloud API ---
async function executeWhatsAppSend() {
    ...
    const hashtagsEl = document.getElementById('WhatsApp-hashtags');
    let captionText = '';
    if (postEl) captionText += postEl.innerText;
    if (hashtagsEl) {
        const pills = hashtagsEl.querySelectorAll('.hashtag-pill');
        const tags = Array.from(pills).map(p => p.textContent).join(' ');
        if (tags) captionText += '\n\n' + tags;
    ...
    if (sendBtn) {
        ...
        sendBtn.innerHTML = '<div class="spinner-small"></div> <span>Sending...</span>';
    }
    if (sendResultEl) sendResultEl.style.display = 'none';
    ...
    try {
        ...
        if (data.error) {
            if (sendResultEl) {
                ...
                sendResultEl.innerHTML = `
                    <div style="background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); border-radius: 10px; padding: 12px; margin-top: 12px; color: #ef4444;">
                        <strong>Send Failed:</strong> ${escapeHtml(data.error)}
                    </div>`;
            }
        } else {
            if (sendResultEl) {
                sendResultEl.style.display = 'block';
                sendResultEl.innerHTML = `
                    <div style="background: rgba(34,197,94,0.1); border: 1px solid rgba(34,197,94,0.3); border-radius: 10px; padding: 12px; margin-top: 12px; color: #22c55e;">
                        <strong>✅ Message Sent Successfully!</strong>
                        <br><small style="color: var(--color-text-muted);">Message ID: ${escapeHtml(data.message_id || 'N/A')}</small>
                    </div>`;
            }
            // Clear phone input for next send
        ...
        if (sendResultEl) {
            ...
            sendResultEl.innerHTML = `
                <div style="background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); border-radius: 10px; padding: 12px; margin-top: 12px; color: #ef4444;">
                    <strong>Error:</strong> ${escapeHtml(err.message || 'Network error')}
                </div>`;
        }
    } finally {
        if (sendBtn) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check WhatsApp Business connection status on load
    checkWhatsAppStatus();
});
