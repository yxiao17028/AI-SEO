...
# Login flow (identity only — openid, email, profile)
...
def auth_google_login():
    ...
    session['state'] = state
    session['auth_flow_type'] = 'login'
    return redirect(authorization_url)

# GSC connect flow (separate — webmasters.readonly)
...
def auth_google():
    ...
    session['state'] = state
    session['auth_flow_type'] = 'gsc'
    return redirect(authorization_url)

@app.route('/auth/google/callback')
def auth_google_callback():
    flow_type = session.pop('auth_flow_type', 'login')
    scopes = LOGIN_SCOPES if flow_type == 'login' else GSC_SCOPES
    ...
    if flow_type == 'login':
        ...
        try:
            ...
            print(f"[{datetime.datetime.now()}] Failed to fetch user info: {e}")
            session['user_email'] = 'unknown@user'
            session['user_name'] = 'User'
            session['user_picture'] = ''
    else:
        # GSC connect flow — store GSC credentials
...
def auth_status():
    ...
    gsc_creds = get_credentials()
    gsc_connected = gsc_creds is not None and gsc_creds.valid
    result = {
        "authenticated": gsc_connected,
        "logged_in": logged_in,
    }
    if logged_in:
        result["user"] = {
...
@app.route('/api/auth/disconnect-gsc')
def auth_disconnect_gsc():
...
def get_current_user_id():
    # Use Google email as stable user identifier
    if 'user_email' in session:
        return session['user_email']
    # Fallback for edge cases (should not happen with login gate)
...
def ai_social_content():
    ...
    - WhatsApp: Direct, personal message style. Keep it concise (under 1000 chars). Conversational and friendly. Include a clear call-to-action. Format for easy reading in a chat bubble.
...
# --- WhatsApp Business API (Embedded Signup + Cloud API) ---

@app.route('/api/whatsapp/connect', methods=['POST'])
def whatsapp_connect():
    """Exchange OAuth code from Embedded Signup for access token and store WABA credentials."""
    req_data = request.json or {}
    code = req_data.get('code', '')
    waba_id = req_data.get('waba_id', '')
    phone_number_id = req_data.get('phone_number_id', '')
    
    if not code:
        return jsonify({"error": "Authorization code is required"}), 400
    if not waba_id or not phone_number_id:
        return jsonify({"error": "WABA ID and Phone Number ID are required"}), 400
    
    if not META_APP_ID or not META_APP_SECRET:
        return jsonify({"error": "Meta App credentials not configured on server"}), 500
    ...
    try:
        ...
        token_resp = requests.get(token_url, params=token_params, timeout=30)
        token_data = token_resp.json()
        
        if "error" in token_data:
            err_msg = token_data['error'].get('message', str(token_data['error']))
            print(f"[WhatsApp Connect] Token exchange error: {err_msg}")
            return jsonify({"error": f"Token exchange failed: {err_msg}"}), 400
        
        access_token = token_data.get('access_token', '')
        if not access_token:
            return jsonify({"error": "No access token received from Meta"}), 400
        
        # Fetch phone number display info
        display_phone = ''
        business_name = ''
        try:
            phone_info_url = f"https://graph.facebook.com/v21.0/{phone_number_id}"
            phone_resp = requests.get(phone_info_url, params={
            ...
            print(f"[WhatsApp Connect] Phone: {display_phone}, Business: {business_name}")
        except Exception as e:
            print(f"[WhatsApp Connect] Could not fetch phone info: {e}")
        
        # Register phone number for Cloud API messaging
        try:
            register_url = f"https://graph.facebook.com/v21.0/{phone_number_id}/register"
            register_resp = requests.post(register_url, json={
                "messaging_product": "whatsapp",
                "pin": "123456"
            }, headers={"Authorization": f"Bearer {access_token}"}, timeout=15)
            register_data = register_resp.json()
            print(f"[WhatsApp Connect] Register response: {register_data}")
        except Exception as e:
            print(f"[WhatsApp Connect] Phone registration note: {e}")
        ...
        cursor.execute('DELETE FROM whatsapp_accounts WHERE user_id = ?', (user_id,))
        cursor.execute('''
            INSERT INTO whatsapp_accounts (user_id, waba_id, phone_number_id, access_token, display_phone, business_name)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (user_id, waba_id, phone_number_id, access_token, display_phone, business_name))
        conn.commit()
        conn.close()
        
        print(f"[WhatsApp Connect] Successfully connected WABA {waba_id} for user {user_id}")
        return jsonify({
            "success": True,
            "waba_id": waba_id,
...
def whatsapp_status():
    """Check if current user has a connected WhatsApp Business Account."""
    user_id = get_current_user_id()
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT waba_id, phone_number_id, display_phone, business_name, connected_at FROM whatsapp_accounts WHERE user_id = ?', (user_id,))
        row = cursor.fetchone()
        conn.close()
        ...
        if row:
            return jsonify({
                ...
                "display_phone": row[2] or '',
                "business_name": row[3] or '',
                "connected_at": row[4] or ''
            })
        else:
...
def whatsapp_send():
    """Send a WhatsApp message with image + caption via Cloud API."""
    req_data = request.json or {}
    recipient_phone = req_data.get('phone', '').strip()
    caption_text = req_data.get('caption', '').strip()
    image_b64 = req_data.get('image_b64', '')
    image_mime = req_data.get('image_mime', 'image/png')
    
    if not recipient_phone:
        return jsonify({"error": "Recipient phone number is required"}), 400
    
    # Clean phone number (remove spaces, dashes, + prefix handling)
    recipient_phone = recipient_phone.replace(' ', '').replace('-', '').replace('(', '').replace(')', '')
    if recipient_phone.startswith('+'):
        recipient_phone = recipient_phone[1:]
    
    # Get stored WABA credentials
    user_id = get_current_user_id()
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT phone_number_id, access_token FROM whatsapp_accounts WHERE user_id = ?', (user_id,))
        row = cursor.fetchone()
        conn.close()
    except Exception as e:
    ...
    if not row:
        return jsonify({"error": "No WhatsApp Business Account connected. Please connect first."}), 400
    ...
    try:
        if image_b64:
            ...
            filename = f"social_content.{ext}"
            
            upload_url = f"https://graph.facebook.com/v21.0/{phone_number_id}/media"
            files = {
                'file': (filename, image_bytes, image_mime)
            }
            upload_data = {
                'messaging_product': 'whatsapp',
                'type': image_mime
            ...
            upload_resp = requests.post(upload_url, headers=api_headers, files=files, data=upload_data, timeout=30)
            upload_result = upload_resp.json()
            
            if 'error' in upload_result:
                err_msg = upload_result['error'].get('message', str(upload_result['error']))
                print(f"[WhatsApp Send] Media upload error: {err_msg}")
                return jsonify({"error": f"Image upload failed: {err_msg}"}), 400
            
            media_id = upload_result.get('id', '')
            if not media_id:
                return jsonify({"error": "Failed to get media ID from upload"}), 500
            
            print(f"[WhatsApp Send] Uploaded media ID: {media_id}")
            
            # Step 2: Send image message with caption
            send_url = f"https://graph.facebook.com/v21.0/{phone_number_id}/messages"
            # Truncate caption to 1024 chars (WhatsApp limit)
            ...
            send_resp = requests.post(send_url, headers={**api_headers, "Content-Type": "application/json"}, json=message_payload, timeout=30)
            send_result = send_resp.json()
            
            if 'error' in send_result:
                err_msg = send_result['error'].get('message', str(send_result['error']))
                print(f"[WhatsApp Send] Send error: {err_msg}")
                return jsonify({"error": f"Message send failed: {err_msg}"}), 400
            ...
            print(f"[WhatsApp Send] Message sent successfully. ID: {msg_id}")
...
def whatsapp_disconnect():
    """Remove stored WhatsApp Business Account credentials."""
    user_id = get_current_user_id()
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        conn.commit()
        conn.close()
        print(f"[WhatsApp Disconnect] Removed WABA for user {user_id}")
        return jsonify({"success": True})
    except Exception as e:
        print(f"[WhatsApp Disconnect] Error: {e}")
        return jsonify({"error": str(e)}), 500
